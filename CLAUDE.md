...
**`docs/internals/` is the change-safety reference** — read [`docs/internals/README.md`](docs/internals/README.md) before modifying any subsystem. It documents each building block's invariants, dependencies, and "change hazards" (what breaks elsewhere when you touch X), grounded in `file:line`. One doc per subsystem (pipeline, mux, demux, poll-tagged, stream-transforms, server-tun, icmp, drivers-proto, modules-cli).

⚠️ **The top-level `README.md` and `docs/mux-tag-poll.md` have drifted from the code — trust the source, not the prose.** Confirmed stale claims: the frame header is **2-byte** (`uint16`), not 4-byte; the constructor is `NewFrameConn` (not `NewFramedConn`) and there is no `WithMaxFrameSize`/`ErrFrameTooLarge`/`maxsize` param; buffered options are `WithBufRead`/`WithBufWrite` (not `WithBufSize`/`WithBuf{Reader,Writer}Size`); `Mux` (server) produces a `TaggedConn`, not a single `net.Conn`; there is no `WithPollBufSize`; and the advertised `reality` driver is not registered. See the "Documentation drift" section of `docs/internals/README.md` for the full list.
...
`go.work` ties together independently-versioned modules. **Each is published and versioned separately** — changing a shared type in the root module means bumping the dependency in every dependent module's `go.mod` (root is currently v1.4.0, `proto/*` v1.1.x, `drivers/*` v1.1.x). The CI release is tagged `cli/vX.Y.Z`.
...
2. In an `init()`, call `netx.Register("<name>", func(params, listener bool) (netx.Wrapper, error) { ... })`. Validate params, branch on `listener` for server vs client, set only the relevant `Wrapper` function fields.
...
- Driver param keys are lowercased and trimmed during parse; unknown params should return an error, not be silently ignored (see existing drivers).
